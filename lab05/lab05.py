...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    lan_a = get_lat(city_a)
    lan_b = get_lat(city_b)
    lon_a = get_lon(city_a)
    lon_b = get_lon(city_b)
    return sqrt((lan_a-lan_b)**2+(lon_a-lon_b)**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_goal = make_city('city_goal', lat, lon)
    if distance(city_a, city_goal) > distance(city_b, city_goal):
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        label_of_tree = tree(label(t))
        for leave in leaves:
             label_of_tree += [[leave]]
        return label_of_tree
    else:
